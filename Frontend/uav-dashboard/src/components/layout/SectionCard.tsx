...
export function SectionCard({
  ...
  return (
    ...
    >
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-[var(--color-border-subtle)] px-5 py-4 sm:px-6">
        <div className="space-y-1">
          ...
          <h2 className="font-[var(--font-display)] text-[1rem] font-semibold tracking-[-0.03em] text-[var(--color-text-primary)]">
            {title}
          </h2>
          {description ? (
      ...
      <div className={`px-5 py-5 sm:px-6 sm:py-6 ${contentClassName}`.trim()}>{children}</div>
    </section>
  )
}
