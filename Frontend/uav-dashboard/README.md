...
- real upload / processing progress while the user stays on the loading screen
- 3D viewer with route replay and time scrubbing
- trajectory color modes for `stage`, `speed`, and `time`
...
- redirect back to landing with a clear error if a requested `flightId` does not exist
...
- Upload progress bars for transport and backend processing
...
- Uploads stay on the loading screen until the backend job is fully completed and the flight is ready to open.
